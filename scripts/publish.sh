...
if ! bash "$SCRIPTS_DIR/validate.sh" > /dev/null 2>&1; then
  error "Validation failed. Run 'scripts/validate.sh' to see details."
fi
...
# Run build.sh to ensure generated files are fresh
info "Building fresh artifacts..."
if ! bash "$SCRIPTS_DIR/build.sh" > /dev/null 2>&1; then
  error "Build failed. Run 'scripts/build.sh' to see details."
fi
ok "Build completed"
...
# Commit version bump and any build changes
...
if [[ $? -eq 0 ]]; then
  ...
  warn "GitHub release creation may have failed"
fi
